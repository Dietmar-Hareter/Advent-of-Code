...
if (FALSE) {
  ...
  point_in_polygon <- function(x0, y0, px, py) {
    # px, py: Vektoren der Polygonpunkte (gleiche Länge)
    # x0, y0: zu testender Punkt
    ...
    for (i in 1:n) {
      ...
      # Prüfe, ob die Kante den horizontalen Strahl schneidet
      intersects <- ((y1 > y0) != (y2 > y0)) && 
  ...
  for (ir in 1:n_row) {
    ...
    if (start$x == end$x) {
      ...
      inner_point <- point_in_polygon(start$x + 1, y, data_new[,1], data_new[,2])
      if (inner_point) { 
        tmp <- (data_new[,1] > start$x) & (data_new[,2] <= y_max) & (data_new[,2] >= y_min)
        x_min <- min(data_new[tmp,1])
        tmp <- (data_new[,1] >= start$x) & (data_new[,1] <= x_min) & (data_new[,2] <= y_min)
        part_min <- max(data_new[tmp,2])
        tmp <- (data_new[,1] >= start$x) & (data_new[,1] <= x_min) & (data_new[,2] >= y_max)
        part_max <- min(data_new[tmp,2])
        area <- (x_min - start$x + 1) * (part_max - part_min + 1)
      } else {
        tmp <- (data_new[,1] < start$x) & (data_new[,2] <= y_max) & (data_new[,2] >= y_min)
        x_max <- max(data_new[tmp,1])
        tmp <- (data_new[,1] <= start$x) & (data_new[,1] >= x_max) & (data_new[,2] <= y_min)
        part_min <- max(data_new[tmp,2])
        tmp <- (data_new[,1] <= start$x) & (data_new[,1] >= x_max) & (data_new[,2] >= y_max)
        part_max <- min(data_new[tmp,2])
        area <- (start$x - x_max + 1) * (part_max - part_min + 1)
      }
    } else {
      x <- (start$x + end$x) / 2
      ...
      inner_point <- point_in_polygon(x, start$y + 1, data_new[,1], data_new[,2])
      if (inner_point) { 
        tmp <- (data_new[,2] > start$y) & (data_new[,1] <= x_max) & (data_new[,1] >= x_min)
        y_min <- min(data_new[tmp,2])
        tmp <- (data_new[,2] >= start$y) & (data_new[,2] <= y_min) & (data_new[,1] <= x_min)
        part_min <- max(data_new[tmp,1])
        tmp <- (data_new[,2] >= start$y) & (data_new[,2] <= y_min) & (data_new[,1] >= x_max)
        part_max <- min(data_new[tmp,1])
        area <- (part_max - part_min + 1) * (x_min - start$y + 1)
      } else {
        tmp <- (data_new[,2] < start$y) & (data_new[,1] <= x_max) & (data_new[,1] >= x_min)
        y_max <- max(data_new[tmp,2])
        tmp <- (data_new[,2] <= start$y) & (data_new[,2] >= y_max) & (data_new[,1] <= x_min)
        part_min <- max(data_new[tmp,1])
        tmp <- (data_new[,2] <= start$y) & (data_new[,2] >= y_max) & (data_new[,1] >= x_max)
        part_max <- min(data_new[tmp,1])
        area <- (part_max - part_min + 1) * (start$y - y_max + 1)
      }
    }
    if (area > max_area) {
      max_area <- area
